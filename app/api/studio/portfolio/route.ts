...
function computeStageGate(
  ...
  // 2. Clear Market Pull -> BUILD
  if (podScore >= 75 && (visitors >= 100 || preorders >= 2 || pir >= 2.0)) {
    const preorderNote = preorders > 0 ? ` with ${preorders} paid reservations (${pir}% PIR)` : "";
    return {
      verdict: "BUILD",
...
export async function GET(request: NextRequest) {
  try {
    ...
    for (const p of projects) {
      ...
      const cvr = totalVisitors > 0 ? Number(((totalLeads / totalVisitors) * 100).toFixed(1)) : 0;
      const pir = totalVisitors > 0 ? Number(((totalPreorders / totalVisitors) * 100).toFixed(1)) : 0;

      const dynamicScore = p.podScore > 0
        ? p.podScore
        : Math.min(100, Math.max(25, Math.round(cvr * 4 + pir * 10 + (totalVisitors > 50 ? 25 : 10))));
      ...
      const targetSlug = p.landingPages[0]?.slug || (p.experiments[0]?.landingPages[0]?.slug) || "preview";
      ...
      concepts.push({
        ...
        slug: targetSlug,
        status: p.status,
        stage: verdict === "BUILD" ? "Validated" : verdict === "KILL" ? "Archived" : "Testing",
    ...
    // If fewer than 3 concepts exist in workspace, enrich with benchmark studio concepts
      ...
      const benchmarkConcepts: StudioConcept[] = [
        {
          ...
          pir: 2.6,
          verdict: "BUILD",
          verdictReason: "Exceptional demand signal (84/100) with 9 paid reservations ($1.00 hold). High willingness to pay confirmed across engineering leads.",
        ...
        },
        {
          ...
          pir: 0.5,
          verdict: "ITERATE",
          verdictReason: "Good traffic interest but low willingness to pay at current $29/mo pricing. Recommend testing micro-tier ($9/mo) or bundling accounting.",
        ...
        },
        {
          ...
          pir: 0.0,
          verdict: "KILL",
          verdictReason: "Failed Stage-Gate after 240 visitors. Under 2% conversion and zero deposit commitment. Reallocate engineering capital.",
    ...
    // Sort by PoD Score descending (Leaderboard order)
    ...
    const avgPodScore = concepts.length > 0
      ? Math.round(concepts.reduce((acc, c) => acc + c.podScore, 0) / concepts.length)
      : 0;

    const avgPir = concepts.length > 0
      ? Number((concepts.reduce((acc, c) => acc + c.pir, 0) / concepts.length).toFixed(1))
      : 0;

    const totalCapitalSaved = concepts.reduce((acc, c) => acc + c.capitalSaved, 0);
    ...
    return NextResponse.json({ error: "Failed to fetch studio portfolio" }, { status: 500 });
  }
}
