        ...
        <div className="grid md:grid-cols-3 gap-6 relative">
          ...
          {stages.map((stage, i) => {
            ...
            const colorMap: Record<string, { border: string; bg: string; icon: string; dot: string; glow: string }> = {
              blue: { border: "border-blue/15", bg: "bg-blue/3", icon: "text-blue bg-blue/8", dot: "bg-blue", glow: "shadow-blue/5" },
              green: { border: "border-green/15", bg: "bg-green/3", icon: "text-green bg-green/8", dot: "bg-green", glow: "shadow-green/5" },
              amber: { border: "border-amber/15", bg: "bg-amber/3", icon: "text-amber bg-amber/8", dot: "bg-amber", glow: "shadow-amber/5" },
            };
            ...
            return (
              <motion.div key={stage.num} initial={{ opacity: 0, y: 40 }} animate={isInView ? { opacity: 1, y: 0 } : {}} transition={{ delay: 0.3 + i * 0.15, duration: 0.6, ease: [0.23, 1, 0.32, 1] }}>
                <TiltCard className="h-full">
                  <div className={`rounded-2xl border ${c.border} ${c.bg} p-8 h-full hover:border-opacity-40 transition-all duration-300 hover:shadow-xl ${c.glow} bg-white shadow-sm`}>
                    ...
                    <div className="border-t border-border/30 pt-4">
                      ...
                      <p className="text-sm font-semibold">{stage.output}</p>
                    </div>
                  </div>
                </TiltCard>
              </motion.div>
